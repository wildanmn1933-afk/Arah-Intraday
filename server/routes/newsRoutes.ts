...
newsRouter.post('/trigger', async (req, res) => {
  try {
    ...
    // 2. Otherwise: Fetch the FRESHEST live post directly from Telegram channels!
    // This scrapes live public Telegram wire (@financialjuice, @SM_News_24h, @WatcherGuru, @fxstreetforexindonesia, @cointelegraph)
    const tgResult = await TelegramIngestionService.fetchNextLiveTelegramNews(category);

    if (!tgResult.isNew || !tgResult.news) {
      return res.json({
        success: true,
        isNew: false,
        message: 'Tidak ada berita baru dari saluran Telegram saat ini. Semua berita terbaru sudah terkirim.',
        news: null,
        event: null,
